...
# lsp: 진단, 서식, 정의, 설명, 끝내기
(cd "$HERE/lsp" && python3 client.py "$M") > "$T/lsp.txt" 2>&1
...
"$M" add loud "file://$T/lib" --rev v1 > /dev/null && [ "$("$M" run main.skn)" = "HI!" ] && ok || bad "패키지 add/run"
...
cd "$T/app" && "$M" add soft "file://$T/lib2" > /dev/null \
...
mkdir -p "$T/reg/packages" && cd "$T/reg" && git init -q -b main \
  && printf 'git = "file://%s/lib"\ndescription = "Loud text"\n' "$T" > packages/loud.toml \
  && git add -A && git -c user.email=t@t -c user.name=t commit -qm init
cd "$T" && "$M" new app2 > /dev/null && cd app2 && printf 'import loud\nfn main():\n    print(loud.shout("hey") + "\\n")\n' > main.skn
export SISKIN_HOME="$T/home"
SISKIN_REGISTRY="file://$T/reg" "$M" search loud | grep -q "Loud text" && ok || bad "패키지 목록 search"
SISKIN_REGISTRY="file://$T/reg" "$M" add loud > /dev/null && [ "$("$M" run main.skn)" = "HEY!" ] && ok || bad "패키지 목록 add"
SISKIN_REGISTRY="file://$T/reg" "$M" add lod 2>&1 | grep -q "did you mean: loud" && ok || bad "패키지 목록 비슷한 이름"
unset SISKIN_HOME
